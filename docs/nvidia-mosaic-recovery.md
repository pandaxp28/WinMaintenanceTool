...
Mosaic は、複数モニターを 1 つの大きなデスクトップとして扱う NVIDIA 機能です。Quadro / RTX + Sync 系の構成では、Mosaic は表示同期や構成管理の一部として使われます。

### 2. MultiMonitorTool.exe
...
Mosaic 解除後、モニター自体は認識されていても、Windows 側で無効や半端な状態のまま残ることがあります。そのため、表示デバイスを起こし直す役割として使います。
...
- `configureMosaic.exe` : Mosaic を解除する
- `MultiMonitorTool.exe` : モニターを有効化する
- `DisplaySwitch.exe` : Windows の表示モードを拡張へ戻す
...
```text
C:\NVIDIA\Tools\configureMosaic.exe
C:\NVIDIA\Tools\MultiMonitorTool.exe
C:\YourFolder\MosaicRecovery.bat
```
...
1. `configureMosaic.exe` と `MultiMonitorTool.exe` を `C:\NVIDIA\Tools` に置く
2. `MosaicRecovery.bat` を保存する
3. バッチを右クリックして **管理者として実行** する
4. 実行後、同じフォルダの `logs` にログが保存される
...
このページの復旧バッチは **表示を戻す処置** です。

一方、TDR は **ドライバ監視の待ち時間設定** です。

つまり役割が違います。

- Mosaic 復旧バッチ : 画面崩れ後の復旧
- TDR 値調整 : ドライバリセットの発生条件の緩和
...
- `TdrLevel=0` のような **完全無効化** は、このページでは推奨しません
...
Quadro / RTX Sync 系の構成では、Mosaic は単なる多画面結合ではなく、同期構成の一部として使われることがあります。NVIDIA の Quadro Sync II ガイドでも、Mosaic は同期設定の前に構成し、その後にフレームロックを有効化する順序が示されています。fileciteturn3file0

この順序性があるため、表示が崩れた場合は、

1. NVIDIA 側の Mosaic を解除
2. 表示デバイスを再有効化
3. Windows 側の表示モードを戻す
